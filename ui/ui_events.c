...
static void selectNatureSoundTask(void *pvParameter) {
    ...
    int id = -1;
    for (int i = 0; i < 4; i++) {
        if(strncmp(utf8_file_names[music_files_count + bath_files_count + i] + 2, sound_name, 3) == 0) {
            sscanf(utf8_file_names[music_files_count + bath_files_count + i], "%2d", &id);
            break;
        }
    }
    if (id == -1) {
